...
仓库使用 **[Changesets](https://github.com/changesets/changesets) + tag-driven 发布** 的组合：

- **版本管理** 由 [`.github/workflows/version.yml`](./.github/workflows/version.yml) 自动化：每次 push 到 `main`，如果有未消费的 `.changeset/*.md`，机器人会开/更新一个 "Version Packages" PR，里面包含版本号 bump + `CHANGELOG.md` 更新。
- **真正的发布** 由 [`.github/workflows/release.yml`](./.github/workflows/release.yml) 完成：当 Version PR 被合并，`version.yml` 会自动创建并 push `v<version>` tag，触发 `release.yml` 跑 OIDC + provenance + smoke test 后发到 npm。

### 日常流程

```bash
# 1. 完成一段开发后，记录变更（在功能分支即可）
npm run changeset
# → 交互式选 patch / minor / major + 填一句话描述
# → 在 .changeset/ 下生成一个 md 文件

git add .changeset/ && git commit -m "chore: add changeset"
git push
# 发起 PR 合并到 main

# 2. 合并到 main 后，version.yml 自动开一个 "Version Packages" PR
#    PR 内容：package.json 版本号 bump + CHANGELOG.md 更新 + 删除已消费的 changeset 文件

# 3. Review 并合并 "Version Packages" PR
#    → version.yml 自动 push v<新版本号> tag
#    → release.yml 触发，发布到 npm
```

整条链路无需手动 `npm version` 或手动打 tag。
...
预发布走 Changesets 的 [pre mode](https://github.com/changesets/changesets/blob/main/docs/prereleases.md)：

```bash
# 进入 pre mode（选 next / beta / rc / alpha 之一作为标识）
npx changeset pre enter next

# 像平时一样添加 changeset、合并 PR
npm run changeset
# → 合并到 main → Version PR → 合并 → tag v0.2.0-next.0 → release.yml 发到 dist-tag=next

# 准备发正式版时退出 pre mode
npx changeset pre exit
```

用户安装预发布版本：`npx @iskl/dev-journal@next install`

### 应急：跳过 changeset 直接发版

如果遇到需要紧急发布、又来不及走 Version PR 的情况，仍可走老路径——手动改 `package.json` 版本号 + 打 tag：

```bash
# ⚠️ 不要用 `npm version`：它会触发我们的 `version` script (= changeset version)，导致再 bump 一次
# 直接编辑 package.json 的 version 字段，然后：
git commit -am "chore: release vX.Y.Z"
git tag vX.Y.Z
git push --follow-tags
```

`release.yml` 仍会接管发布。但常态请走 changeset 流程，保持 CHANGELOG 完整。
